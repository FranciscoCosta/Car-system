...
const handleState = (e) => {
  ...
  if (status === 'Todos') {
    ...
    const filteredOrders = totalOrders.filter((order) => order.status === status)
...
const handleDate = (e) => {
  ...
  console.log(date)
  const filteredOrders = totalOrders.filter((order) => order.deliveryDate === date)
  setOrders(filteredOrders)
}
          ...
          <div className="MechanicOrders__container-items-list">
            {orders.map((order) => (
              <div className="MechanicOrders__container-items-list-item">
                  <span
                  style={{backgroundColor: order.status === 'Pendente' ? '#e40145' : order.status === 'Em andamento' ? '#ff9600' : '#00c21d'}}
                  >{order.status}</span>
                  <h3>Preço: <p>{order.price}</p></h3>
                  <h3>Data de entrega: <p>{order.deliveryDate}</p></h3>
                  <h3>Descrição: <p>{order.description}</p></h3>
                  <AiFillMessage/>
          </div>
          ))}
          </div>
